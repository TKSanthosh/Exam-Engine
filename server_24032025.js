...
const { DASHBOARD_CONFIG } = require("./exam_dashboard_api/dashboardConstants"); 
const refreshToken = require("./exam_dashboard_api/refreshToken");
const axiosPrivate = require("./exam_dashboard_api/axiosPrivate");
//package ends
...
app.post("/clientlogin", async (req, res) => {
  try {
    ...
    const apiToken = jwt.sign({ service: "authService" }, JWT_SECRET, {
     ...
     // JWT DASHBOARD API
    const loginResponse = await axiosPrivate.post('/examEngineLogin', JSON.stringify({
      username: username,
    ...
    const insertRefreshToken = async (centreCode,serialNo,refreshToken,dbExpiryTime) =>
      {
       return new Promise((resolve, reject) => {
      db.query(
        "INSERT INTO hybrid_mern.refresh_tokens (centre_code, serial_no, token, expires_at) VALUES (?, ?, ?, ?)",[centreCode, serialNo, refreshToken, dbExpiryTime],
        (err, result) => (err ? reject(err) : resolve())
      );
    });
  }
   await insertRefreshToken(username, serialnumber, refreshTokens, dbExpiryTime);
    // await db.query("INSERT INTO hybrid_mern.refresh_tokens (centre_code, server_no, serial_no, token, expires_at) VALUES (?, ?, ?, ?, ?)", [centreCode, serverNo, serialNo, refreshToken, dbExpiryTime]);
