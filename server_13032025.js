...
// setInterval(updateSystemIp, 5000); // Update every 5 seconds
...
let io = socketIo(server, {
  cors: {
    origin: ['http://localhost','http://localhost:3000','http://localhost:3001'],  // Allow your frontend domain
    // origin: ['http://localhost','http://localhost:3000',`${ipFile.API_URL}`,`${ipFile.API_URL}:3000`],  // Allow your frontend domain
    methods: ['GET', 'POST'],
...
const tablesToExport = [
  "autofeed","biometric_report_api","descriptive_answer","exam_skip_biometricvalidation","iib_candidate_iway",
  "iib_candidate_scores","iib_candidate_test","iib_candidate_tracking","iib_feedback","iib_response",
  "iib_section_test","netboot_ip_mapping",
  "timelog",
];
...
app.use(
  cors({
    origin: [ 'http://localhost:3000', 'http://localhost','http://localhost:3000' ],
...
app.get("/update-env/:ip",(req,res)=>{
  ...
  const newEnvFile = envFile.replace(/^REACT_APP_API_URL=.*/m, `REACT_APP_API_URL=http://${ip}`);
...
app.post("/clientlogin-old", (req, res) => {
  ...
  axios.post("http://localhost:5002/checkUser",
    ...
    .then((response) => {
      ...
      if (response.data?.response_status === "1") {
        ...
        db.query(sqlCheck, [username, "D"], (err, results) => {
          ...
          const formattedsqlInsert = db.format(sqlInsert, [username,"a","Mac Address","D",formattedTime]);
          db.query(sqlInsert,
...
app.post("/clientlogin", async (req, res) => {
  try {
    ...
    const response = await axios.post(
      `${process.env.DEMOSERVER}/exam-dashboard/autoAssignServerNumber`,
      data_eal
    );
    ...
    if (status_eal === "F" || status_eal === "NF") {
      ...
      return res.status(500).json({ message: errorMessages[error_type] || "Invalid login" });
    }

    if (status_eal !== "M") {
      return res.status(500).json({ message: "Not valid build. Please contact Help desk." });
    }
    ...
    if (count == 0) {
      ...
      const responseServerReady = await axios.post(
        `${process.env.DEMOSERVER}/exam-dashboard/biometricCountUpdateApi`,
        dataServerReady
      );

      const responseServerReadyVal = responseServerReady.data.message.split("--");
      if (responseServerReadyVal[0] !== "S") {
        return res.status(500).json({ message: "Server ready status not updated. Please contact admin." });
      }
    ...
    res.status(200).json({
      message: count === 0 ? "Login successful and record inserted" : "Login successful, record already exists",
      token: clientToken,
    });
  } catch (error) {
...
app.post("/Qpactivation", async (req, res) => {
  ...
  const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/autoAssignServerNumber`,(data_eal),
...
if (status_eal === "S") {
  try {
    ...
    console.log("==================================================================");
      await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/getServerFiles`,
...
//     // const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/getServerFiles`,(data_ealThree), {
//     //   responseType: "arraybuffer", // Ensures binary data is received
//     //   httpsAgent: new https.Agent({ rejectUnauthorized: false }), // Ignores SSL errors if any
//     // });
...
if(status=="Act"){
  ...
  if(status == "Base" && qpStatus == 1){
    ...
    console.log(data_eal);
  const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/dataDownload`,(data_eal),
    // {headers: 
  ...
  const responseOne = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/dataDownload`,(data_ealOne),
    // {headers: 
    //   { Authorization: `Bearer ${apiToken}`,"Content-Type": "application/json",},
    //       withCredentials: true,}
        );
  ...
  const responseTwo = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/dataDownload`,(data_ealTwo),
    // {headers: 
    //   { Authorization: `Bearer ${apiToken}`,"Content-Type": "application/json",},
    //       withCredentials: true,}
        );
...
if(responseCurlOne=="A"){
  ...
  const filename = Buffer.from(`${Buffer.from(dPathOne, "base64").toString()}/${sqlDB}.zip||${process.env.DB_NAME}`).toString("base64");
  ...
  const extractPath = process.env.DOWNQPPATH; // Extract directly here
    ...
    // let dumpFilePath = downQPpath1;
try{
  const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/getServerFiles`, { pathcmd: filename } , {
...
if (response.status !== 200) {
  ...
  return res.status(response.status).json({success:"false",message:"Failed to download the file."});
}
...
try {
  ...
  console.log("ONE",res_ealOne);
    ...
    try {
      ...
      const responseStatus = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/dataDownload`,(data_ealStatus),
        // {headers:
        //   { Authorization: `Bearer ${apiToken}`,"Content-Type": "application/json",},
        //       withCredentials: true,}
      );
...
app.post("/insert-base", async (req, res) => {
  try {
    ...
    const serialNumber = utils.getSerialNumber();
    ...
    db.query(checkSql, checkValues, (err, checkResult) => {
      if (err) {
        console.error("MySQL select error:", err);
        return res.status(500).json({ message: "Error checking data in the database." });
      }
      ...
      const downloadValues = [centre_code, serverno, download_sec, "D", formattedTime];
      const formattedinsertDownloadSQL = db.format(insertDownloadSQL, downloadValues);
      db.query(insertDownloadSQL, downloadValues, (err) => {
        if (err) {
          console.error("MySQL insert error:", err);
          return res.status(500).json({ message: "Error inserting data into qp_download." });
        }
      ...
      // });
          ...
          axios
            .post(`${process.env.DEMOSERVER}/exam-dashboard/autoAssignServerNumber`, data_eal)
            .then((response) => {
...
app.get("/clear", (req, res) => {
  const queries = [`DROP DATABASE ${process.env.DB_NAME}`, `CREATE DATABASE ${process.env.DB_NAME}`];
...
app.post("/response", (req, res) => {
  const {questionId,answer,qpno,displayorder,tag,hostip,updatedtime,clienttime,totalTime, encryptKey} = req.body;
  ...
  db.beginTransaction(async (err) => {
    ...
    try {
      ...
      const formattedInsertResponseSql = db.format(insertResponseSql, [qpno,questionId,quesType === "DQ" ? "DQ" : answer,encryptKey,displayorder,tag,hostip,updatedtime,clienttime]);
      ...
      if (quesType === "DQ") {
        const insertDqQuery =
          ...
          const formattedinsertDqQuery = db.format(insertDqQuery, [lastInsertedId, questionId, qpno, answer]);

        await new Promise((resolve, reject) => {
          db.query(insertDqQuery,[lastInsertedId, questionId, qpno, answer, encryptKey],(err) => {
              if (err) {
                return reject(
    ...
    const formattedUpdateTestSql = db.format(updateTestSql, [ updatedtime, time_taken, clienttime, clienttime, hostip, qpno, hostip, qpno]);
...
app.post("/update-exam-status", (req, res) => {
  const {membershipNo,exam_code,subject_code,score,exam_date,time_taken,result,auto_submit,updated_on} = req.body;
  ...
  db.beginTransaction((err) => {
    ...
    const formattedinsertScoreSql = db.format(insertScoreSql, [membershipNo,exam_code,subject_code,score,exam_date,time_taken,result,auto_submit,updated_on,]);
    ...
    db.query(formattedinsertScoreSql, (err, result) => {
      ...
      const updateTestSql ="UPDATE iib_candidate_test SET  test_status = ?, end_time = ? WHERE membership_no = ? and subject_code = ?";
      const formattedupdateTestSql = db.format(updateTestSql, ["C",updated_on,membershipNo,subject_code,]);
      db.query(
        updateTestSql,
...
app.post("/submit-form", (req, res) => {
  ...
  const sql ="INSERT INTO user_data (name, email, password, subject_code, exam_date) VALUES (?, ?, ?, ?, ?)";
  const formattedsql = db.format(sql, [name,email,hashedPassword,subjectCode,examDate,]);
  db.query(
    sql,
...
app.post("/submit-qp", (req, res) => {
  ...
  questions.forEach((question) => {
    const formattedsql = db.format(sql, [subjectCode,question.question,question.options[0].text,question.options[1].text,question.options[2].text,question.options[3].text,question.correct_ans,question.mark,
    ]);

    db.query(
      sql,
      [subjectCode,question.question,question.options[0].text,question.options[1].text,question.options[2].text,question.options[3].text,question.correct_ans,question.mark,],
      (err, result) => {
        if (err) {
          console.error("MySQL insert error:", err);
          res.status(500).json({ message: "Internal Server Error" });
...
app.get("/initialAnswers/:questionPaperNo/:encryptKey", async (req, res) => {
  ...
  const getCorrectAns = async (answer, quesId, qpno) => {
    if (answer === "DQ") {
      ...
      return new Promise((resolve, reject) => {
        db.query(query, [encryptKey, quesId, qpno, quesId, qpno], (err, result) => {
          if (err) {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (recordExists) {
        ...
        queryParams = [text,formattedTime,membership_no,question_paper_no,subject_code,exam_date,];
      } else {
        // Insert new record
        ...
        queryParams = [membership_no,question_paper_no,subject_code,exam_date,text,formattedTime,formattedTime,];
      }
...
app.post("/talogin", (req, res) => {
  ...
  const checkExamSql = "SELECT * FROM exam_closure_summary WHERE centre_code= ?";
  db.query(checkExamSql, [center_code], (err, resultExam) => {
    ...
    if (resultExam.length > 0) {
      return res.status(402).json({ success: false, message: "Exam completed!" });
    }

    const loginSql = "SELECT * FROM iib_ta_details WHERE ta_login = ? AND ta_password = PASSWORD(?)";
    db.query(loginSql, [username, password], (err, results) => {
      ...
      if (results.length > 0) {
        trackTALogin(username, center_code, hostIP, 1, (trackErr) => {
          if (trackErr) return handleError(res, "Error tracking TA login", trackErr);
...
function trackTALogin(taLogin, cafeID, hostIP, sessionID, callback) {
  ...
  const formattedInsertQuery = db.format(sqlInsert, [taLogin, cafeID, hostIP, sessionID]);
   // Insert the exact formatted query into xml_feed
...
function trackCandidateLogin(membershipNo,taLogin,cafeID,hostIP,sessionID,examCode,subjCode,browser,callback) {
  ...
  db.query(checkQuery, [taLogin, membershipNo, examCode, subjCode], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      db.query(updateQuery, [formattedTime, taLogin, membershipNo, examCode, subjCode], (updateErr) => {
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const credentialsResult = await new Promise((resolve, reject) => {
      db.query(sqlCheckCredentials, [username, password, centre_code], (err, results) => {
        if (err) return reject(err);
        resolve(results);
      });
    });
    ...
    if (
      ...
      trackCandidateLogin(username,ta_login,hostIP,hostIP,1,exam_code,subject_code,browser,
        (trackErr) => {
          if (trackErr) return handleError(res, "Error during candidate login tracking", trackErr);
          res.json({ success: true, message: "Login successful" });
        }
      );
    } else {
...
app.post("/get-candidate-test-status", (req, res) => {
  ...
  db.query(checkQuery, [membership_no, exam_code, subject_code], (err, results) => {
    if (err) {
...
app.post("/update-candidate-test-status", (req, res) => {
  ...
  db.query(checkQuery, [membership_no, exam_code, subject_code], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      db.query(updateQuery, [formattedTime, membership_no, exam_code, subject_code], (updateErr) => {
        if (updateErr) {
...
app.post("/update-exam-date", (req, res) => {
  ...
  const formattedupdateQuery = db.format(updateQuery, [examDate,examDate,examDate]);
...
app.get("/handleBatchClosure/:batchId/:hostIp/:serialNumber/:centreCode",
  async (req, res) => {
    const { batchId, hostIp, serialNumber, centreCode } = req.params;
    console.log("batchId:", batchId,"hostIp:", hostIp,"serialNumber:", serialNumber,"centreCode:", centreCode);
    try {
      ...
      await queryAsync(insertSummaryQuery, [centreCode,batchId,zipFileName,serialNumber,formattedDate,formattedDate,hostIp,]);

      const formattedinsertSummaryQuery = db.format(insertSummaryQuery, [formattedDate,centreCode,batchId,zipFileName,serialNumber,formattedTime,formattedTime,hostIp,]);
...
//           const {questionPaperNo,membershipNo,examCode,subjectCode,passMark,roundoff_score,graceMark,timeTaken,} = candidate;
...
//               generateScoreForCandidate(questionPaperNo,membershipNo,examCode,subjectCode,passMark,roundoff_score,graceMark,timeTaken,"Y",
//                 (err, score) => {
//                   if (err) return reject(err);
//                   resolve(score);
//                 }
...
//                             (exam_date, centre_code, serverno, closure_batch_time, closure_batch_file, closure_batch_status, serial_no, updated_on, added_on, ip_address) 
//                             VALUES (?, ?, 'a', ?, ?, 'I', ? , ?, ?, ?)`;
//               await queryAsync(insertSummaryQuery, [formattedDate,centreCode,batchId,zipFileName,serialNumber,formattedDate,formattedDate,hostIp,]);

//               const formattedInsertResponseSql = db.format(insertSummaryQuery, [formattedDate,centreCode,batchId,zipFileName,serialNumber,formattedTime,formattedTime,hostIp,]);
...
//                 const insertExamSummaryQuery = `INSERT INTO exam_closure_summary (exam_date, centre_code, serverno, closure_action, closure_status, added_on, ip_address) VALUES (?, ?, 'a', 'Sync Process Data', 'I', ?, ?)`;
...
//         const insertSummaryQuery = `INSERT INTO batchwise_closure_summary (exam_date, centre_code, serverno, closure_batch_time, closure_batch_file, closure_batch_status, serial_no, updated_on, added_on, ip_address) VALUES (?, ?, 'a', ?, ?, 'I', ? , ?, ?, ?)`;
//         await queryAsync(insertSummaryQuery, [formattedDate,centreCode,batchId,zipFileName,serialNumber,formattedTime,formattedTime,hostIp,]);
...
//               centreCode,
//             ]);
//             const formattedupdateExamSummaryQuery = db.format(updateExamSummaryQuery,[zipClosureFileName, centreCode]);
...
async function generateScoreForCandidate(
  ...
  const {posscore,negscore,score,examResult} = await utils.getScoreCalculation(questionPaperNo,encryKey,roundoff_score,graceMark,passMark);
  ...
  console.log(`Final score for candidate ${membershipNo}: ${score}`);
          ...
          db.query(
            ...
            (err, rowsScoreCheck) => {
              ...
              const queryParams =
                cnt_score_chk === 0
                  ? [membershipNo,examCode,subjectCode,score,formattedTime,timeTaken,examResult,auto_submit,]
                  : [score,formattedTime,timeTaken,examResult,auto_submit,membershipNo,examCode,subjectCode,];

              const formattedinsertOrUpdateScoreQuery = db.format(insertOrUpdateScoreQuery,[queryParams]);
...
//           AND AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256))) between AES_DECRYPT(A.option_1,UNHEX(SHA2(?, 256))) and AES_DECRYPT(A.option_2,UNHEX(SHA2(?, 256)))
//           AND A.question_type = 'R'
//           AND (AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256)))!='' or AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256)))!='NULL');
//     `;
...
//              AND AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256))) not between AES_DECRYPT(A.option_1,UNHEX(SHA2(?, 256))) and AES_DECRYPT(A.option_2,UNHEX(SHA2(?, 256)))
...
//               AND AES_DECRYPT(A.option_1,UNHEX(SHA2(?, 256))) = AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256)))  
...
//         WHERE B.question_paper_no = ?
//           AND B.id IN (SELECT MAX(id) FROM iib_response WHERE question_paper_no = ? GROUP BY question_id)
//           AND A.correct_answer = AES_DECRYPT(B.answer,UNHEX(SHA2(?, 256)))
...
app.post("/getScoreOfcandidate",async (req,res)=>{
  const {questionPaperNo,encryKey,roundoff_score,graceMark,passMark} = req.body;
...
//   const query = `SELECT  a.marks,a.correct_answer,AES_DECRYPT(b.answer,?),AES_DECRYPT(a.option_1,?),AES_DECRYPT(a.option_2,?),a.question_type FROM iib_sq_details A LEFT JOIN iib_response B ON A.question_id=B.question_id where B.question_paper_no = ? AND B.id IN (SELECT MAX(id) FROM iib_response WHERE question_paper_no = ? and AES_DECRYPT(b.answer,?) !='NULL' GROUP BY question_id) `;

// }
  
app.get("/handleDayClosure/:batchId/:hostIp/:serialNumber/:centreCode",
  async (req, res) => {
    const { batchId, hostIp, serialNumber, centreCode } = req.params;
                ...
                if(processDataResult == "Process Data successfully completed"){
                  ...
                  if(processFeedResult=="Feed process completed"){
                    ...
                    if(processSyncResult=="Sync Process completed"){
                      ...
                      if(processdownloadDBDump == "Dump Downloaded and synced to the central server"){
                        return res.json({message: "Day closure processed successfully",processStatus: 1});
                      }
...
// async function processData(center_code, serverno, database, closure_exam_date, ip_addr, value, curlpath_process_data) {
//   try {
//     let msg = '', backup_status = '', countfile = 0;
//     const date_time = new Date().toISOString().replace(/[-T:.Z]/g, '');
//     const lbl = 'Process Data';

//     // if (!checkExamTime(center_code)) {
//     //   return "Please click Process Data button once the exam time has been completed";
//     // }
...
//     const filename = path.join(value, feedname);
...
//       // const createFeed = Buffer.from(row[1].trim()).toString('base64') + ";\n\n";
//       // fileHandle.write(createFeed);
//       const createFeed = Buffer.from(row.query.trim()).toString('base64') + ";\n\n";
//       fileHandle.write(createFeed);
//     }
...
//     const reqChkSumVal = crypto.createHash('md5').update(reqChkSumStr + process.env.CHECKSUMKEY).digest('hex');
...
//     // const content = await curlCall(curlpath_process_data, data);
//     // const initiate_process_status = content.trim() === 'FILE UPLOAD SUCCESS' ? 'U' : 'UF';
//     const response = await axios.post(curlpath_process_data,(data_eal),
//     // {headers: 
//     //   { Authorization: `Bearer ${apiToken}`,"Content-Type": "application/json",},
//     //       withCredentials: true,}
//         );
...
//     const process_zip_path = path.join(value, process_zip_name);
...
//     const selectQuery = "SELECT id FROM exam_closure_summary WHERE centre_code=? AND serverno=? AND closure_action=?";
...
//       await new Promise((resolve, reject) => {
//         db.query(insertQuery, [closure_exam_date, center_code, serverno, process_zip_name, lbl, backup_status, formattedTime, ip_addr], (err) => {
//           if (err) return reject(err);
//           resolve();
//         });
//       });
...
//       await new Promise((resolve, reject) => {
//         db.query(updateQuery, [process_zip_name, backup_status, formattedTime, ip_addr, center_code, serverno, lbl], (err) => {
//           if (err) return reject(err);
//           resolve();
//         });
//       });
...
async function processFeed(center_code, serverno, database, closure_exam_date, ip_addr, curlpath_feed) {
  try {
    ...
    const remove_exist_zip = path.join(__dirname, 'feed', `feed_${database}_${center_code}_${serverno}_*.zip`);
    ...
    // Sync feed data
    const reqChkSumStr = sTaLoginID + sTaPassword + database;
    // const reqChkSumVal = crc32(reqChkSumStr + process.env.CHECKSUMKEY).toString();
    ...
    const checkClosureReport = "SELECT * FROM iib_closure_report WHERE exam_date=?";
    ...
    if (closureReport.length === 0) {
      const insertQuery = "INSERT INTO iib_closure_report (feed_status, db_dump_status, exam_date) VALUES ('1', '0', ?)";
      await new Promise((resolve, reject) => {
      ...
      const updateQuery = "UPDATE iib_closure_report SET feed_status='1' WHERE exam_date=?";
      await new Promise((resolve, reject) => {
    ...
    const checkClosureSummary = "SELECT id FROM exam_closure_summary WHERE centre_code=? AND serverno=? AND closure_action=?";
    const closureSummary = await new Promise((resolve, reject) => {
      db.query(checkClosureSummary, [center_code, serverno, lbl], (err, res) => {
        if (err) return reject(err);
        resolve(res);
      });
    });
    ...
    if (closureSummary.length === 0) {
      const insertClosureSummary = "INSERT INTO exam_closure_summary (exam_date, centre_code, serverno, closure_action, closure_status, added_on, ip_address) VALUES (?, ?, ?, ?, 'D', ?, ?)";
      await new Promise((resolve, reject) => {
        db.query(insertClosureSummary, [closure_exam_date, center_code, serverno, lbl,formattedTime, ip_addr], (err) => {
          if (err) return reject(err);
          resolve();
        });
      });
    } else {
      const updateClosureSummary = "UPDATE exam_closure_summary SET closure_status='D', updated_on = ?, ip_address=? WHERE centre_code=? AND serverno=? AND closure_action=?";
      await new Promise((resolve, reject) => {
        db.query(updateClosureSummary, [formattedTime, ip_addr, center_code, serverno, lbl], (err) => {
          if (err) return reject(err);
          resolve();
        });
      });
...
async function processSync(center_code, serverno, database, closure_exam_date, ip_addr, curlpath_sync_data) {
  try {
    ...
    const dmppath = path.join(__dirname, file_ecs_process);
    ...
    // const response = await axios.post(curlpath_sync_data, data);
    // const content = response.data.trim();
    // console.log("Sync Response:", content);
    const content = 'FILE UPLOAD SUCCESS';
    ...
    if (existingSync.length === 0) {
      ...
      await new Promise((resolve, reject) => {
        db.query(insert_qp_dl, [closure_exam_date, center_code, serverno, lbl, sync_status, formattedTime, ip_addr], (err) => {
          if (err) return reject(err);
          resolve();
        });
      });
      ...
      await new Promise((resolve, reject) => {
        db.query(update_qp_dl, [sync_status, formattedTime, ip_addr, center_code, serverno, lbl], (err) => {
          if (err) return reject(err);
          resolve();
        });
      });
    ...
    return sync_status === 'U' ? 'Syncing success' : `${content} Syncing failed`;
  } catch (error) {
...
async function downloadDBDump(center_code, serverno, database, assign_serial_no, attend_candidate_count, curlpath_feed, closure_exam_date, ip_addr, todaydate, value) {
  try {
      if (!center_code || !serverno) {
          return "QP not downloaded yet";
      }
    ...
    const { ta_login: sTaLoginID, ta_password: sTaPassword } = taDetails;
      ...
      const date_time = new Date().toISOString().replace(/[-T:.Z]/g, '');
      const dmppath = path.join(value, `${database}_${center_code}_${serverno}_${date_time}_${assign_serial_no}.dmp`);
      const backup_tables_str = (Array.isArray(tablesToExport) && tablesToExport.length) ? tablesToExport.join(' ') : '';
      ...
      const existingRecord = await new Promise((resolve, reject) => {
        db.query("SELECT id FROM qp_download WHERE centre_code = ? AND serverno = ? AND download_sec = 'Post Exam QP'", 
      ...
      if (!existingRecord) {
        insert_id = await new Promise((resolve, reject) => {
          db.query("INSERT INTO qp_download (centre_code, serverno, download_sec, download_status, download_time) VALUES (?, ?, 'Post Exam QP', 'D', ?)", 
            [center_code, serverno, formattedTime], (err, res) => {
              if (err) return reject(err);
      ...
      let isCompleteDump = fileLines.slice(-6).some(line => /^-- Dump completed on/i.test(line));

      if (isCompleteDump) {
          await db.query("UPDATE qp_download SET download_status = 'DV' WHERE id = ?", [insert_id]);
      }
      ...
      if (isCompleteDump) {
          ...
          const data = {
              ...
              CHECKSUM: reqChkSumVal,
              file: `@${dmppath}`,
              attended_candidate: attend_candidate_count
          };
          ...
          if (content === 'FILE UPLOAD SUCCESS') {
              await db.query("UPDATE qp_download SET download_status = 'U' WHERE id = ?", [insert_id]);
              
              const existingReport = await db.query("SELECT * FROM iib_closure_report WHERE exam_date = ?", [todaydate]);
              
              if (existingReport.length === 0) {
                  await db.query("INSERT INTO iib_closure_report (feed_status, db_dump_status, exam_date) VALUES (0, 1, ?)", [todaydate]);
              } else {
                  await db.query("UPDATE iib_closure_report SET db_dump_status = 1 WHERE exam_date = ?", [todaydate]);
              }

              return "Dump Downloaded and synced to the central server";
          } else {
              await db.query("UPDATE qp_download SET download_status = 'UF' WHERE id = ?", [insert_id]);
              return content + ' QP downloaded but Dump Syncing failed';
          }
      } else {
          await db.query("UPDATE qp_download SET download_status = 'DC' WHERE id = ?", [insert_id]);
...
function getCandidateAttendExam() {
  return new Promise((resolve, reject) => {
    const query = "SELECT COUNT(DISTINCT membership_no) AS count FROM iib_candidate_test";
  ...
  db.query(checkQuery, [membership_no, question_paper_no], (err, results) => {
    ...
    db.query(formattedInsertQuery, (err, insertResults) => {
      if (err) {
        ...
        return res.status(500).json({ error: "Error inserting candidate test data." });
      }
      ...
      db.query(updateQuery, [formattedTime, 1, membership_no, exam_code, subject_code], (updateErr) => {
        if (updateErr) {
          ...
          return res.status(500).json({ error: "Error updating candidate tracking." });
        }
        ...
        insertIntoXmlFeed(formattedInsertQuery, (feedErr) => {
          if (feedErr) {
            ...
            return res.status(500).json({ error: "Error logging query into feed." });
          }

          res.json({
            success: true,
            message: "Data inserted, tracking updated, and query logged successfully.",
...
async function processXmlFeed() {
  ...
  db.query(selectQuery, (err, rows) => {
    ...
    if (rows.length > 0) {
      ...
      const formattedinsertQuery = db.format(insertQuery, [fileName,startId,endId,]);
    ...
    db.query(tableCheckQuery, async (err, tableExists) => {
      ...
      db.query(selectQuery, async (err, rows) => {
        ...
        db.query(sql_tadetails, async (err, results) => {
          ...
          const checkSumValue = sTaLoginID + hashedPassword + process.env.DB_NAME_DASH;

          const validateCheckSum = crypto.createHash('sha256').update(checkSumValue + process.env.CHECKSUMKEY).digest('hex');

          const sTaPassword = crypto.createHash("md5").update(results[0].ta_password).digest("hex");

          for (const feedInfo of rows) {

            // console.log(rows);
            const feedFilename = path.join(process.env.FEED_DIR, feedInfo.filename);
            ...
            const reqChkSumStr = sTaLoginID + sTaPassword + process.env.DB_NAME_DASH;
            ...
            try {
              const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/feed`, form, {
                headers: {
                  ...form.getHeaders(),
                },
              });
              ...
              console.error(`Error sending file ${feedInfo.filename}:`, error.data);
            }
          }
        });
      });
...
const exportTablesAsDump = async (centreCode, serialNumber) => {
  try {
    ...
    try {
      const response = await axios.post(`${process.env.DEMOSERVER}/exam-dashboard/dbDumpFeed`, form, {
        headers: {
          ...form.getHeaders(),
        },
      });
...
app.post("/process-exam-data", async (req, res) => {
  const {submitted,examDate,Exam_Code,Subject_code,memno,filter_by_idle_time} = req.body;

  if ( submitted && examDate && Exam_Code && Subject_code && memno && filter_by_idle_time) {
...
app.get("/feed-list", (req, res) => {
  fs.readdir(feed_path, (err, files) => {
    ...
    const feed_list = files.map((file) => {
      const filePath = path.join(feed_path, file);
              ...
              (err, rows3) => {
                ...
                const processMemberships = rows3.map((row3) => {
                  return new Promise((resolve, reject) => {
                    ...
                    db.query(sqlTimeQuery,[row3.mem_no, examCode, subjectCode],
                      (err, rows4) => {
                        if (err) {
                          reject(err);
                        } else {
...
app.get("/candidate-report/:rollNum", async (req, res) => {
  try {
    ...
    for (const { exam_code: examCode, exam_name: examName } of exams) {
      ...
      for (const { subject_code: subjectCode, subject_name: subjectName, qp_encry_key: encryptKey } of subjects) {
        // Fetch question paper details
        const questionQuery = `
          SELECT question_paper_no
          FROM iib_candidate_test
          WHERE exam_code = ? AND subject_code = ? AND test_status = 'C' AND membership_no = ?`;
        const questions = await queryDB(questionQuery, [examCode, subjectCode, rollNum]);
        ...
        const memberAddress = [
          ...
          member.address4, member.address5, member.address6, member.pin_code
        ].filter(addr => addr).join(" ");
        ...
        const iwayDetails = await queryDB(iwayQuery, [examCode, subjectCode, rollNum]);
        if (iwayDetails.length) {
          const iway = iwayDetails[0];
          iwayCentreCode = iway.centre_code;
          iwayExamTime = iway.exam_time;
          dispExamDate = iway.exam_date ? utils.formatExamDateDMY(iway.exam_date) : "";
        }
        ...
        if (iwayCentreCode) {
          ...
          const iwayAddresses = await queryDB(iwayAddressQuery, [iwayCentreCode]);
          if (iwayAddresses.length) {
            const addr = iwayAddresses[0];
            iwayAddress = [
              addr.iway_address1, addr.iway_address2, addr.iway_city,
              addr.iway_pin_code, addr.iway_state
            ].filter(a => a).join(" ");
          }
        }
        ...
        const mediums = await queryDB(mediumQuery, [rollNum, examCode, subjectCode]);
        if (mediums.length) {
          const { medium_code: mediumCode, institution_name: institutionNameFromDB } = mediums[0];
          strMedium = ["EN", "E", "ENGLISH"].includes(mediumCode) ? "ENGLISH" :
                      ["HI", "H", "HINDI"].includes(mediumCode) ? "HINDI" :
                      "ENGLISH";
        }
        ...
        let unAttQns = 0, attQns = 0;
        let ansQuesAnswer = {}, markedAnswerArray = [];
        ...
        // Fetch question IDs
        const rowsSqlQnsIds = await queryDB(`SELECT question_id FROM iib_question_paper_details WHERE question_paper_no = ? ORDER BY display_order`, [questionPaperNo] );
        ...
        results.push({
          examCode, examName, subjectCode, subjectName, encryptKey,
          memberName: member.name, memberAddress, iwayExamTime,
          strMedium, iwayAddress, totalMarks, passMark,
          iwayCentreCode, dispExamDate, score, institutionName,
          qnsSum, unAttQns, attQns, questionTextArray, aQuestionsLength: qnsSum, markedAnswerArray
        });
      }
    }
...
app.get("/bulk-time-extend/", async (req, res) => {
  ...
  const updateTestWithoutCompleted = (test_id, timeExtensionUpdate) => {
    return new Promise((resolve, reject) => {
      const updateQuery ="UPDATE iib_candidate_test SET time_extended = time_extended + ? WHERE test_id = ?";
        const formattedupdateQuery = db.format(updateQuery, [timeExtensionUpdate, test_id]);
        // Insert update query into xml_feed
        insertIntoXmlFeed(formattedupdateQuery, (err) => {
          if (err) {
            return db.rollback(() => {
  ...
  const updateTestWithCompleted = (test_id, timeExtensionUpdate) => {
    return new Promise((resolve, reject) => {
      const updateQuery ="UPDATE iib_candidate_test SET time_extended = time_extended + ?, test_status = 'IC' WHERE test_id = ?";
      const formattedupdateQuery = db.format(updateQuery, [timeExtensionUpdate, test_id]);
        // Insert update query into xml_feed
        insertIntoXmlFeed(formattedupdateQuery, (err) => {
          if (err) {
  ...
  const deleteCandidateScores = (membership_no, selectedSubject) => {
    return new Promise((resolve, reject) => {
      const deleteQuery =
        ...
        const formatteddeleteQuery = db.format(deleteQuery, [membership_no, selectedSubject]);
        // Insert update query into xml_feed
...
`;
          ...
          return res.json({Questioncount,attendedQusCount,CandidateResponse,displayResponse,});
        }
      }
    }
  } catch (error) {
...
app.post("/exam-closure-summary", (req, res) => {
  ...
  const values = [ExamName,ExamDate,CentreCode,ServerNo,formFields.candidateBatch1Scheduled,formFields.candidateBatch2Scheduled,formFields.candidateBatch3Scheduled,formFields.candidateBatch1Attended,formFields.candidateBatch2Attended,formFields.candidateBatch3Attended,formFields.labsUsed,formFields.testAdministrators,formFields.candidatesWithoutAdmitCard,formFields.candidatesWithoutIdentityProof,formFields.candidatesWithoutAdmitCardAndIdentityProof,formFields.candidatesReportingLate,formFields.candidatesRequestingCentreChange,formFields.candidatesIndulgingInMalpractice,AdminId,currentTimestamp,SerialNumber,"S",];
  ...
  db.query(insertQuery, values, (err, result) => {
    ...
    //   const sTaPassword = crypto.createHash("md5").update(results[0].ta_password).digest("hex");
    //   const reqChkSumVal = validateCheckSum;
    //   const CurrentExamDate = await getCurrentExamDate();
    // // **Create FormData**
    ...
    // form.append("feedbackText", formFields.feedback);
    // form.append("dayEndReport", sTaPassword);
    // form.append("isFileAttached", isFileAttached);
    // form.append("CHECKSUM", reqChkSumVal);
    // form.append("filename", fs.createReadStream(dumpFilePath));
...
app.post("/skip-biometric-validation-all-insert", async (req, res) => {
  const {batchtime,blockmode,biostatus,serialNumber,admin_ipv4,candidateList,} = req.body;
  ...
  db.query(selectSql, queryParams, (err, result) => {
    ...
    if (result.length > 0) {
      ...
      result.forEach((row) => {
        ...
        db.query(checkSql,[row.membership_no, row.exam_date, batchtime],
          (checkErr, checkResult) => {
            if (checkErr) {
              ...
              const updateValues = [blockmode,currentDateTime,biostatus,serialNumber,admin_ipv4,row.membership_no,row.exam_date,batchtime,];
              ...
              const insertValues = [row.exam_date,batchtime,blockmode,row.membership_no,currentDateTime,currentDateTime,biostatus,serialNumber,admin_ipv4,];
              ...
              db.query(insertSql, insertValues, (insertErr, insertResult) => {
                if (insertErr) {
                  ...
                  formattedResult.push({action: "inserted",insertId: insertResult.insertId,membership_no: row.membership_no,exam_date: formattedDateMonthYear,batchtime,});
                }
                processCount++;
                if (processCount === result.length) {
                  res.status(200).json({
...
app.post('/add-seats-allocation', (req, res) => {
  ...
  const values = [exam_centre_code,exam_lab_code,exam_seatno,candidate_ipaddress,status,formattedTime, formattedTime];
  // console.log('Query:', query);
  ...
  try {
    ...
    const insertingInQPTable = async (
      ...
      try {
        const insertingInQPTableQry =
          ...
          const formattedinsertingInQPTableQry = db.format(insertingInQPTableQry, [questionPaperNo,examCode,subjectCode,totalMarks,isSample,choosenMedium]);

        return new Promise((resolve, reject) => {
          db.query(insertingInQPTableQry,
            [questionPaperNo,examCode,subjectCode,totalMarks,isSample,choosenMedium,],
            (err, res) => {
              if (err) {
                console.error("Database query failed:", err);
                return reject(
             ...
             // Insert update query into xml_feed
              insertIntoXmlFeed(formattedinsertingInQPTableQry, (err) => {
                if (err) {
                  console.error("Error inserting update query into xml_feed:", err);
                  return reject(
                    new Error("Error inserting update query into xml_feed")
                  );
                }
    ...
    if (qpStructureLength == 0) {
      ...
      if (listCaseIDArr.length > 0) {
        ...
        const insertIntoQPDetail = await insertIntoQPDetails(insertIntoQPDetailsquery);
        // Insert update query into xml_feed
       await new Promise((resolve, reject) => {
        ...
        const updateMemNoInQP = await updateMemInQP(updateMemInQPquery,membershipNo,questionPaperNo);

        const formattedupdateMemInQPquery = db.format(updateMemInQPquery, [membershipNo,questionPaperNo]);
        await new Promise((resolve, reject) => {
...
app.post("/scannerUpload", (req, res) => {
  ...
  try {
    ...
    if (invalidIP) {
      ...
      return res.status(400).json({ message: `Invalid IP found in file: ${invalidIP}` });
    }
...
app.post("/submit-biometric-ips", (req, res) => {
  ...
  if (validIP) {
    ...
    db.query(queryTruncate, (err) => {
      ...
      const formattedqueryInsert = db.format(queryInsert, [serverIPs, formattedTime]);

      db.query(queryInsert,[serverIPs, formattedTime],
        (err) => {
          if (err) {
      ...
      fs.createReadStream(filePath)
        ...
        .on("data", (line) => {
          if (line) {
            ...
            console.log("Extracted Values:", { membershipNo, seatNo, examDate, batchTime });
            ...
            db.query(checkQuery, [memberNo, batchTime], (err, results) => {
              ...
              if (results.length > 0) {
                ...
                db.query(checkSeatQuery, [seatNo, labname, batchTime], (err, seatResults) => {
                  if (err) throw err;
                  ...
                  if (seatResults.length === 0) {
                    ...
                    const formattedupdateQuery = db.format(updateQuery, [seatNo, formattedTime, memberNo, batchTime]);
                    ...
                    db.query(updateQuery, [seatNo, formattedTime, memberNo, batchTime], (err) => {
                      if (err) throw err;
                      affectedRows++;
                    });
                  } else {
        ...
        .on("end", () => {
          ...
          Object.keys(batchwise).forEach((batchTime) => {
            ...
            db.query(countQuery, [batchTime], (err, results) => {
              ...
              console.log("Exam Values:", { examName, examDate, batchTime, numSeats});
              
              const trackCheckQuery = `
                SELECT id FROM batchwise_tracking WHERE exam_name = ? AND exam_date = ? AND batch_time = ?
              `;

              db.query(trackCheckQuery, [examName, examDate, batchTime], (err, trackResults) => {
                if (err) throw err;
                ...
                if (trackResults.length > 0) {
                  ...
                  db.query(updateTrackQuery, [numSeats, syncStatus, formattedTime, id], (err) => {
                    if (err) throw err;
                  });
                  const formattedupdateTrackQuery = db.format(updateTrackQuery, [numSeats, syncStatus, formattedTime, id]);
                  // Insert update query into xml_feed
                  new Promise((resolve, reject) => {
                  ...
                  const insertTrackQuery = `
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                  `;
                  db.query(insertTrackQuery, [examName, examDate, batchTime, numSeats, syncStatus, formattedTime], (err) => {
                    if (err) throw err;
                  });
                  const formattedupdateinsertTrackQuery = db.format(insertTrackQuery, [examName, examDate, batchTime, numSeats, syncStatus, formattedTime]);
                  // Insert update query into xml_feed
                  new Promise((resolve, reject) => {
                    insertIntoXmlFeed(formattedupdateinsertTrackQuery, (err) => {
                      if (err) {
                        return reject(
                          new Error("Error inserting update query into xml_feed")
                        );
...
function getExamTimeByCandidate(mem_no) {
  return new Promise((resolve, reject) => {
    const query = "SELECT exam_time FROM iib_candidate_iway WHERE membership_no = ?";
...
app.post("/dynamic-sync-biometric-api", async (req, res) => {
  ...
  db.query(query, async (err, results) => {
    if (err || results.length === 0) {
      console.error("Error fetching server IPs or no servers found:", err?.message);
    ...
    for (const serverHost of serverList) {
      try {
        const getExamTime = examTime || (await new Promise((resolve) => {
        ...
        if (parseInt(apiType, 10) === 1 && membershipNo) {
          ...
          if (data !== 2) {
            const executeResponse = await executeXMLData(data, params, serverHost);
...
const executeXMLData = async (xmlData, dataParams, host) => {
  ...
  if (Array.isArray(getQuery.memberNotExist) && getQuery.memberNotExist.length > 0) {
...
const isMemberExists = (membershipNo, examTime) => {
  ...
  return new Promise((resolve, reject) => {
    const query = 'SELECT COUNT(*) AS count FROM biometric_report_api WHERE membership_no = ? AND batch_time = ?';
        ...
        console.log("1",memberExists);
          if (memberExists[1] > 0) {
              ...
              if (api_type === '1') {
                  ...
                  getExamTime = examTime || (await new Promise((resolve) => {
                    getExamTimeByCandidate(membershipNo, resolve);
                  }))
              } else {
              ...
              db.query(chkSeatsQuery, [getExamTime], (err, chkSeatsResult) => {
                  ...
                  if (numSeatsResult > 0) {
                      ...
                      db.query(chkAlreadyAvailQuery, [examName, examDate, batchTime], (err, chkAlreadyAvailResult) => {
                          if (err) return res.status(500).send(err);
                          ...
                          if (chkAlreadyAvailResult.length > 0) {
                              ...
                              const formattedupdateQuery = db.format(updateQuery, [biometricUser, syncBiometricExamStatus,formattedTime, id]);
                              // Insert update query into xml_feed
                              new Promise((resolve, reject) => {
                                insertIntoXmlFeed(formattedupdateQuery, (err) => {
                                  if (err) {
                              ...
                              db.query(updateQuery, [biometricUser, syncBiometricExamStatus,formattedTime, id], (err) => {
                                console.log("updateQuery",{biometricUser, syncBiometricExamStatus,formattedTime, id});
                                  if (err) return res.status(500).send(err);
                              ...
                              const formattedinsertQuery = db.format(insertQuery, [examName, examDate, batchTime, biometricUser, syncBiometricExamStatus, formattedTime]);
                              // Insert update query into xml_feed
                              new Promise((resolve, reject) => {
                                insertIntoXmlFeed(formattedinsertQuery, (err) => {
                                  if (err) {
                              ...
                              db.query(insertQuery, [examName, examDate, batchTime, biometricUser, syncBiometricExamStatus, formattedTime], (err) => {
                                  if (err) return res.status(500).send(err);
                                  return res.send('Success');
                              });
                          }
...
const imageDirectory = "C:\\pro\\itest\\activate\\photos\\questionpaper\\images";
...
app.post("/candidate-time-extend", async (req, res) => {
  const { batchtime, blockmode, serialNumber, candidateList, extentedtime } = req.body;
  ...
  try {
    let finalCandidateList = Array.isArray(candidateList) ? candidateList : [candidateList];
    ...
    if (!finalCandidateList || finalCandidateList.length === 0) {
      return res.status(200).json({ message: "No candidates found for processing." });
    }
    ...
    if (candidateResults.length === 0) {
      return res.status(200).json({ message: "Selected candidate not started the exam" });
    }
    ...
    if (statusC.length > 0) {
      ...
      queries.push(executeQueryWithFeed(updateQueryC, [timeExtensionUpdate, statusC], "update (C status)"));
    }
    ...
    if (statusOther.length > 0) {
      ...
      queries.push(executeQueryWithFeed(updateQueryOther, [timeExtensionUpdate, statusOther], "update (Other status)"));
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    insertIntoXmlFeed(formattedQuery, (err) => {
      ...
      db.query(query, params, (err, result) => {
        ...
        console.log(`${actionLabel} operation affected ${result.affectedRows} rows.`);
...
const loadingActFile = async (escapedPassword,extractedDmp,time)=>{
  ...
  return new Promise((resolve,reject)=>{
    exec(command, (error, stdout, stderr) => {
      ...
      return resolve({success: true,message:"Act file imported successfully",time})
    });
  })
...
app.post("/manualQPUpload", async (req, res) => {
  const updatingPasswordAct = async (time)=>{
    const query = "update iib_candidate set password=password_act where membership_no in (select membership_no from iib_candidate_iway where exam_time=?)";
    return new Promise((resolve,reject)=>[
  ...
  const insertingQPDownload = async (centerCode,serverno,activationText)=>{
    const query = "insert into qp_download (centre_code, serverno, download_sec, download_status, download_time) values (?,?,?,'D',?)";
    return new Promise((resolve,reject)=>{
      db.query(query,[centerCode,serverno,activationText,formattedTime],(err,result)=>{
        if(err){
          console.log(err);
          return reject(err);
        }
  ...
  try {
    ...
    const extractPath = path.join(uploadDir, path.basename(zipFile.name, ".zip")); // Extract folder
    const fileName = (zipFile.name).split(".")[0];
    const batchTime = fileName.split("-")[1];
    const time = batchTime.substring(0, 2) + ":" + batchTime.substring(2, 4) + ":" + batchTime.substring(4, 6);
    ...
    const extractedDmp = path.join(uploadDir,"photos","questionpaper","images",`${fileName}.txt`)
    ...
    const resultJson = await loadingActFile(escapedPassword,extractedDmp,time)
...
const getActiveBatches = (callback) => {
  const activateBatch = [];
  db.query("SELECT REPLACE(download_sec, 'Activated-', '') as active_batch FROM qp_download WHERE download_sec LIKE 'Activated-%' AND download_status = 'D'", (err, results) => {
    if (err) return callback(err);
...
const getTrackedBatches = (database, getExamDate, callback) => {
  const trackBatch = [];
  db.query("SELECT batch_time FROM batchwise_tracking WHERE exam_name = ? AND exam_date = ? AND sync_exam_status = 'C'", [database, getExamDate], (err, results) => {
    if (err) return callback(err);
...
const processBatches = async (mac, database, getExamDate, center_code, serverno, qpDownPath) => {
  getActiveBatches((err, activateBatch) => {
    ...
    getTrackedBatches(database, getExamDate, (err, trackBatch) => {
      ...
      const listSyncBatch = activateBatch.filter(batch => !trackBatch.includes(batch));
      if (listSyncBatch.length === 0) return;

      listSyncBatch.forEach(activeBatch => {
        db.query("SELECT COUNT(DISTINCT a.membership_no) as count FROM iib_candidate_iway a, iib_candidate_test b WHERE a.membership_no = b.membership_no AND a.exam_code = b.exam_code AND a.subject_code = b.subject_code AND a.exam_date = ? AND a.exam_time = ?", [getExamDate, activeBatch], async (err, results) => {
          if (err) return console.error(err);
          ...
          if (batchwiseCandidateCount > 0) {
            ...
            try {
              ...
              if (status == 'S') {
                db.query("SELECT COUNT(1) as count FROM batchwise_tracking WHERE exam_name = ? AND exam_date = ? AND batch_time = ?", [database, getExamDate, activeBatch], (err, results) => {
                  if (err) return console.error(err);
                  
                  if (results[0].count > 0) {
                    db.query("UPDATE batchwise_tracking SET sync_exam_status = 'C' WHERE exam_name = ? AND exam_date = ? AND batch_time = ?", [database, getExamDate, activeBatch]);
                  } else {
                    db.query("INSERT INTO batchwise_tracking (exam_name, exam_date, batch_time, sync_exam_status, added_on, updated_on) VALUES (?, ?, ?, 'C', ?, ?)", [database, getExamDate, activeBatch, formattedTime, formattedTime]);
                  }
                });
              }
            } catch (error) {
...
const syncBiometricExamStatus = async (database, getExamDate, mac, center_code, serverno, qpDownPath) => {
  ...
  db.query(query, [database, getExamDate], async (err, results) => {
    ...
    if (results.length > 0) {
      for (const row of results) {
        const biometricUserCount = row.biometirc_user > 0 ? row.biometirc_user : 0;
        const biometricSkipUserCount = row.biometirc_skip_user > 0 ? row.biometirc_skip_user : 0;
        ...
        try {
          ...
          if (status == 'S') {
            ...
            db.query(updateQuery, [formattedTime, database, getExamDate, biometricBatch], (updateErr) => {
              if (updateErr) {
...
app.get('/dashboardFeedSend', async (req, res) => {
  try {
    ...
    const qpDownPath = `${process.env.DEMOSERVER}/exam-dashboard/biometricCountUpdateApi`;
