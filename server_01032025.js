...
app.use(
  cors({
    origin: [ 'http://localhost:3000', 'http://localhost', 'http://202.144.77.234' ],
    // origin: "*",
...
app.post("/clientlogin", async (req, res) => {
  ...
  const [versionResult] = await new Promise((resolve, reject) => {
    db.query('SELECT id,build_name,build_version FROM taserver_version ORDER BY id desc LIMIT 1', (err, result) => {
      if (err) reject(err);
      else resolve(result);
    });
  });
  const { id: verId, build_name: bname, build_version: bversion } = versionResult;

  // const data_eal = {
  //   m: serialnumber,
  //   d: process.env.DB_NAME_DASH,
  ...
  console.log(process.env.DEMOSERVER);
  try {
    //https://node.sifyitest.com/exam-dashboard/autoAssignServerNumber
    
    // const { data } = await curlCall(`${process.env.DB_NAME}/exam-dashboard/autoAssignServerNumberController`, data_eal);

    // const agent = new https.Agent({ rejectUnauthorized: false }); // Only affects this request
    ...
    if (status_eal == 'F') {
      ...
      emsg = error_type == '1' ? 'Serial number mismatch. Please contact Help desk.'
        : error_type == '2' ? 'Invalid username/password'
          // : error_type == '3' ? 'Not valid build. Please contact Help desk.'
          : error_type == '3' ? 'Build version not found'
            : 'Invalid login';

            return res.status(500).json({ message: emsg });
    } else if (status_eal == 'NF') {
      // emsg = 'Build version not found';
      emsg = 'Not valid build. Please contact Help desk.';
      return res.status(500).json({ message: emsg });
    } else if (status_eal == 'M') {
      succflag = 1;
    } else {
      // const imgfile = `${status_eal}`;
      // const docPath1 = path.join('docPath', path.basename(imgfile));
      // const response = await axios.get(imgfile, { responseType: 'arraybuffer' });

      // fs.writeFileSync(docPath1, response.data);
      // const directory = await unzipper.Open.file(docPath1);
      // await directory.extract({ path: 'docPath' });

      // const updverQry = `UPDATE taserver_version SET build_name='${res_eal[1]}', build_version='${res_eal[2]}', updated_date=${formattedTime} WHERE id=${verId}`;
      // connection.query(updverQry, (err) => { if (err) throw err; });
      // fs.unlinkSync(docPath1);
      // emsg = 'Patch updated successfully. Please login again.';
    }

    if (succflag == 1) {
      // req.session.sid = req.sessionID;
      // req.session.admin_id = login;
      // req.session.admin_type = res_eal[1];

      // res.redirect('/qpupload');
      // return;
      // Check if the record already exists in the database
      const clientToken = jwt.sign({ username, serialnumber }, JWT_SECRET, {
        expiresIn: "24h",
      ...
      db.query(sqlCheck, [username, "D"], (err, results) => {
        ...
        // Insert a new record
        const sqlCheck = `SELECT COUNT(*) AS count FROM qp_download WHERE centre_code = ? AND serverno = ? AND download_sec = ?`;
        db.query(sqlCheck, [username, "a", "Mac Address"], (err, results) => {
          if (err) {
            console.error("MySQL select error:", err);
            return res.status(500).json({ message: "Internal Server Error" });
          }
          ...
          if (results[0].count === 0) {
            // Row doesn't exist, proceed with insertion
            const sqlInsert = `INSERT INTO qp_download (centre_code, serverno, download_sec, download_status, download_time) VALUES (?, ?, ?, ?, ?)`;
            const formattedsqlInsert = db.format(sqlInsert, [username, "a", "Mac Address", "N", formattedTime]);
        
            db.query(sqlInsert, [username, "a", "Mac Address", "N", formattedTime], (err) => {
              if (err) {
                console.error("MySQL insert error:", err);
                return res.status(500).json({ message: "Internal Server Error" });
              }
            ...
            // Row already exists, return success without inserting
            return res.status(200).json({
              message: "Login successful, record already exists",
              token: clientToken,
            });
          }
        });
    ...
    console.error(error);
    // emsg = 'No response for patch';
    res.status(500).json({ message: "No response for patch" });
  }
});
...
try {
  ...
  // const sqlInsert = `INSERT INTO qp_download (centre_code, serverno, download_sec, download_status, download_time) VALUES (?, ?, ?, ?, ?)`;
  // const sqlUpdate = `UPDATE qp_download SET serverno = ? , download_status = ? , download_time = ? WHERE centre_code = ?`;
  const sqlUpdate = `UPDATE qp_download SET serverno = ? , download_status = ? WHERE centre_code = ?`;
  const UpdateVal =[servernoValOne, "D", ccodeOne];
...
//     const sql ="INSERT INTO qp_download (centre_code, serverno, download_sec, download_status, download_time) VALUES (?, ?, ?, ?, ?)";
//     const values = [centre_code, serverno, download_sec, "D", formattedTime];

//     const autoID=1;

//     const sql_autofeed_truncate = "TRUNCATE TABLE autofeed";


//     const parts = download_sec.split("-");
//     // Output: ['Activated', '10:00:00']
//     const activate_status = parts[0]; // 'Activated'
//     const batch_time = parts[1];    // '10:00:00'
//     // Truncate Table First
//     db.query(sql_autofeed_truncate, (err, result) => {
//       if (err) {
//         console.error('Error truncating table:', err);
//         return;
//       }
//        // Insert the exact formatted query into xml_feed
//        insertIntoXmlFeed(formattedsql_autofeed, (err) => {
//         if (err) {
//           return db.rollback(() => {
//             console.error("Error inserting feed table:", err);
//             res.status(500).json({ message: "Internal Server Error" });
...
//       db.query(sql_autofeed, values_autofeed, (err, result) => {
//         if (err) {
//           console.error("MySQL insert error:", err);
//           return res.status(500).send("Error inserting data into the database.");
//         }
...
//       // Need to work on mock and autoID
//       // if (autoID !== "" && mockDB === "") {
//       //   if (autoID !== "") {
//       //   const queries = [
//       //     `ALTER TABLE iib_candidate_test AUTO_INCREMENT = ${autoID}`,
...
//     db.query(checkSql, checkValues, async (err, result) => {
//       if (err) {
//         console.error("MySQL select error:", err);
...
//       const insertDownloadSQL = "INSERT INTO qp_download (centre_code, serverno, download_sec, download_status, download_time) VALUES (?, ?, ?, ?, ?)";
//       const downloadValues = [centre_code, serverno, download_sec, "D", new Date()];
...
//         // Truncate autofeed
//         db.query("TRUNCATE TABLE autofeed", (err) => {
//           if (err) {
//             console.error("Error truncating table:", err);
//             return res.status(500).json({ message: "Error truncating autofeed table." });
//           }
...
app.get("/exam-data/:centrecode", async (req, res) => {
  try {
    ...
    ON slot.membership_no = test.membership_no WHERE slot.centre_code = ? GROUP BY slot.exam_date`;
...
app.post("/response", (req, res) => {
  ...
  db.beginTransaction(async (err) => {
    ...
    try {
      ...
      const updateTestSql = `UPDATE iib_candidate_test SET last_updated_time = ?, time_taken = ?, time_left = ?, clienttime = ?  WHERE host_ip = ? AND question_paper_no = ? AND start_time = (SELECT MAX(start_time) FROM iib_candidate_test WHERE host_ip = ? AND question_paper_no = ?)`;
...
app.get("/initialAnswers/:questionPaperNo/:encryptKey", async (req, res) => {
  ...
  const getCorrectAns = async (answer, quesId, qpno) => {
    if (answer === "DQ") {
      const query = `SELECT CONVERT(AES_DECRYPT(desc_ans, ?) USING 'utf8') AS desc_ans FROM descriptive_answer WHERE question_id = ? AND question_paper_no = ? AND response_id = (SELECT MAX(response_id) FROM descriptive_answer WHERE question_id = ? AND question_paper_no = ?)`;
  ...
  const sql = `SELECT r1.display_order,CONVERT(AES_DECRYPT(CAST(r1.answer AS BINARY), ?) USING 'utf8') AS answer, r1.tag, r1.question_id, r1.question_paper_no FROM iib_response AS r1 WHERE r1.question_paper_no = ? AND r1.id = (SELECT MAX(r2.id) FROM iib_response AS r2 WHERE r2.question_id = r1.question_id AND r2.question_paper_no = ?)`;
    ...
    for (const rowExam of rowsExam) {
      ...
      for (const rowSubject of rowsSubject) {
        ...
        for (const rowSelQues of rowsSelQues) {
          ...
          const sqlQns = `SELECT question_id, CONVERT(AES_DECRYPT(answer, ?) USING 'utf8') AS answer, display_order FROM iib_response WHERE id IN (SELECT MAX(id) FROM iib_response WHERE question_paper_no = ? AND answer != 'NULL' GROUP BY question_id ) ORDER BY display_order`;
