...
const RenderHtmlContent = ({ htmlString, caseId, caseText, questionType,incrementingId,renderedOptions }) => {
  ...
  return (
    <>
      {questionType === "DQ" ? (
        <div>
          <div style={{ marginLeft: "26px", marginTop: "-23px" }} className={`ques${incrementingId}`} id={`desc${incrementingId}`}>
            {processedContent}
          </div>
        </div>
      ) : caseId > 0 ? (
        <div className="row">
        <div className="row" style={{ marginTop: "-23px" , flexDirection:"column"}}>
          ...
          <div className="col-12 col-md-5">
            <div style={{ marginLeft: "26px" }} className={`ques${incrementingId}`}>
              {processedContent}
              {questionType=="O" ? renderedOptions : null}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginLeft: "26px", marginTop: "-23px" }} className={`ques${incrementingId}`}>
          {processedContent}
          {renderedOptions}
        </div>
      )}
