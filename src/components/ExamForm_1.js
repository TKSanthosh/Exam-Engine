...
const ExamSubmitConfirmation = ({ open, onClose, onConfirm, timeLeft }) => {
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      <DialogTitle sx={{ textAlign: "center", bgcolor: "#f5f5f5" }}>Confirmation</DialogTitle>
      <DialogContent sx={{ textAlign: "center", px: 3, bgcolor: "#f5f5f5" }}>
        <DialogContentText>
          You have <strong>{formatTime(timeLeft)}</strong> hrs to answer  
          <strong> {document.getElementById("UnattemptedQus")?.value} </strong> un-answered question(s),  
          <strong> {document.getElementById("TaggedQus")?.value} </strong> tagged question(s),  
          <strong> {document.getElementById("TaggedAttemptedQus")?.value} </strong> tagged & attempted question(s).
          <br /><br />
          Still, do you want to proceed with your submission?
        </DialogContentText>
      </DialogContent>
      <DialogActions sx={{ justifyContent: "center", pb: 2, bgcolor: "#f5f5f5" }}>
        <Button 
          onClick={onClose} 
          sx={{ px: 1, py: 1, backgroundColor: "#90c723", color: "white", '&:hover': { backgroundColor: "#88b20f" } }}
        >
          No
        </Button>
        <Button 
      ...
      <>
        ...
        >
          ...
          <span className="mark-label">
            (Marks: {currentQuestion.mark}) | (Negative Marks:{(currentQuestion.negative_mark != '' && currentQuestion.negative_mark != 'NULL' && currentQuestion.negative_mark != null ) ? currentQuestion.negative_mark : 0  })
            ...
            <div className="containers">
              ...
              <div className="main">
                ...
                <span className="qtext" style={{ fontSize: `${fontSize}px` }}>
                  <input type="hidden" id="caseIdVal" value={currentQuestion.case_id} ></input>
        ...
        <small>({maxLength - dqAnswer.length} characters remaining)</small>
      </div>
                    <TextField
                      label="Answer"
                      className="mt-3 mx-2"
                  ...
                  <tr>
                    <td colSpan={6}>
                      ...
                      >
                        ...
                        <tbody>
                          {CandidateResponse.map((response, index) => {
                            if (index % 3 === 0) {
                              return (
                                <tr key={index}>
                                  {[0, 1, 2].map((offset) => {
                                    const currentIndex = index + offset;
                                    if (currentIndex < CandidateResponse.length) {
                                      return (
                                        <React.Fragment key={currentIndex}>
                                          ...
                                          <td className="greybluetext10">
                                            {CandidateResponse[currentIndex]["answer"] &&
                                            CandidateResponse[currentIndex][
                                              "answer"
                                            ] !== "NULL" ? (
                                              <b>
                                                {
